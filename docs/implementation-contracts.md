...
상태: 구현 전 계약 초안 — 확정된 의사결정 반영
관련 결정: [MVP 의사결정 대장](decision-register.md)

이 문서는 API, 영속성, 작업자, Codex 담당자가 서로 다른 가정을 하지 않도록 모듈 사이의 경계를 정의합니다. 결정 대장의 `ACCEPTED` 값이 우선하며, 아래 테이블은 아직 만들지 않은 마이그레이션의 목표 스키마입니다.
...
- 모든 내부 ID는 UUIDv4이며 DB에는 PostgreSQL `uuid`로 저장합니다.
...
- 첫 마이그레이션에서는 PostgreSQL 열거형 또는 `CHECK` 제약 가운데 하나로 통일합니다. 임의의 문자열 입력은 허용하지 않습니다.
...
- 여러 작업자는 `FOR UPDATE SKIP LOCKED`를 사용해 같은 작업을 동시에 가져가지 않습니다.
...
- 기본 동시 실행 수는 1입니다. `PENDING`과 `RUNNING`을 합친 미완료 작업은 최대 100개이며, 새 사건과 작업을 추가할 때는 접수 트랜잭션에서 이 상한만을 위한 고정된 PostgreSQL 자문 잠금을 잡고 미완료 작업 수를 센 뒤 삽입합니다. 동시 접수도 이 순서를 따라야 합니다. 상한에 도달하면 이벤트를 저장하지 않고 `503`을 반환합니다. 중복 이벤트는 기존 사건 ID와 현재 상태를 반환합니다.
